...
class PvP(commands.Cog):
	...
	async def end_battle(self,battle,winner):
		...
		if winner.id == battle.user1.id or winner.id == battle.user2.id:
			if winner.id == battle.user1.id: coins = battle.team_hp[1]*REWARD_MOD#user 1 won. calculate player 2 power
			else: coins = battle.team_hp[0]*REWARD_MOD#user 2 won. calculate player 1 power
		...
		if reaction.message.author.id == BOT_ID and user.id != BOT_ID:
			...
			if color == discord.Colour(16776961) and str(reaction) == u'\u2705':#react to fight request
				...
				if battle.locks[0] == battle.locks[1]:#both are true
					...
					await battle.users[1-(id-1)].send("The other user has completed their character selection. You have 180 seconds before the selection period ends. If you do not choose your cards before this time period expires you will automatically lose")
			...
			elif color == discord.Colour(7895161) or color == discord.Colour(7895162) or color == discord.Colour(7895163):#Select move for char 1, 2 and 3
				#checks
				
				battle, id = self.return_battle(user)
				
				if str(reaction) == u"\u2694":#attack
					card = battle.inst_card[id-1][battle.player_pointer[id-1]-1]
					
					if card[6] == ["Reload","self_status",40,False,6,"Reload Shell's for the Shrine Tank","loaded",-10] and "loaded" not in card[4]:
					...
					for emoji in targets:#Add only reactions for characters alive
					...
					if points >= spell[2]:#cost of spell
						...
						await user.send("You dont have enough power to cast this spell")		
				
			elif color == discord.Colour(7895164):#select target for attack
				battle, id = self.return_battle(user)
				action = battle.waiting_target[id-1]
				pointer = battle.player_pointer[id-1]-1#What character are we currently talking about
				if str(reaction) == u"\U0001F1E6":
					action[2] = (pointer,0)
				elif str(reaction) == u"\U0001F1E7":
					action[2] = (pointer,1)
				elif str(reaction) == u"\U0001F1E8":
					action[2] = (pointer,2)
				...
				num = int(str(reaction).strip("\u20E3")) - 1#number of reaction clicked
	...
	async def select(self, ctx, ID):
		...
		if battle == None:
			...
			await ctx.send("You already have 3 characters selected. Use +remove to take a card off of the selection before trying to add another card")
			return
	...
	async def adventure(self,ctx, page = 0):
		...
		embed.add_field(name = ":one: Sakuya's Dojo - Selectable difficulty",value = "First Time Completion Bonus: None",inline = False)
		embed.add_field(name = "---------------------------",value = "---------------------------",inline = False)
		embed.add_field(name = ":three: Scarlet Devil Mansion - Hard",value = "First Time Completion Bonus: WIP",inline = False)
	...
	async def fight(self, ctx, user:discord.Member):
		...
		trade_embed = discord.Embed(
			...
			description = "<@{}>, <@{}> has requested to fight against you. React with \u2705 to start the battle.".format(user.id,sender.id)
		)
...
class Battle(spellcard_func.Spells):
	...
	async def add_char(self,char,id):
		...
		new_embed.add_field(name="Character {}: ".format(len(new_embed.fields)+1),value=str(char[2]) + "	| Lv" + str(char[3]) + "	| " + str(char[5]) + "	| " + str(char[6]) + "	| " + str(char[7]) + "	| " + ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][char[4]-1],inline = False)
	...
	async def start(self):
		...
		for player in self.user_cards:
			...
			for card in player:#creates instance of card
				...
				try:
					...
					spell = ["Generic Spell Card","single_target_dmg",100,True,10,"A spell given to characters without a spell card. Deals damage to a single target",0.4]
		...
		if self.pve == False:#only send msg to user 2 if its not a pve battle
	...
	async def next_command(self,id):
		cards = self.inst_card[id]
		
		try:
			while "dead" in cards[self.player_pointer[id]][4]:#keep incrementing until pointed char is not dead or we go beyond the list (No chars alive left to do actions)
				self.player_pointer[id] += 1
		except Exception as e:
			pass
	
		if self.player_pointer[id] > 2:#third command already chosen
			self.locks[id] = True
			...
			if self.locks[0] == self.locks[1]:#both player have selected all their moves
				...
				if self.winner != None:#there is a winner
			...
			if self.pve:
				...
				ldict = {"moves":self.actionBar,"level":int(self.stage),"chars":self.inst_card,"points":self.points}
		...
		msg = await self.users[id].send(embed = embed)
		self.msg[id] = msg
		await msg.add_reaction(u"\u2694")
	...
	def generate_command_embed(self,id):
		...
		card = self.user_cards[id][self.player_pointer[id]]
			...
			for card in player:
				...
				if "vamp" in card[4]:
					...
					if card[8][1] <= 0:
						...
						card[2] += atkup
						self.battle_log.append("{}s attack went up by {}[10%] due to rage status".format(card[5],atkup))
				...
				for i in card[8][2:]:#loop through all timed effects not in main categories
					...
					if i[1] == "hp_regen":
						...
						if hp_regen > 0:
							self.battle_log.append("{} regained {} HP".format(card[5],hp_regen))#Dont show this message if no healing is done
	...
	async def update_log(self,img=None):#Updates battle log to whats currently in the Battle_log list
		...
		log = ""
		for line in self.battle_log[-LOG_SIZE:]:#get last x values of battle_log and loop through them where x is LOG_SIZE (started off as 10)
			log += line+"\n"
	...
	#card (pvp) => [#0 = hp,1 = atk, 2 = spd,3 = defending, 4 = statuses, 5 = nickname, 6 = spell card, 7 = Max hp, 8 = status value]
		...
		self.points[move[4]] += int((dmg/self.total_hp) * 600 * GIVEN_MOD * Attacker_multiplier)#attacker gains points
		self.points[target_player] += int((dmg/self.total_hp) * 600 * (1+1*target_card[3]) * TAKEN_MOD * Defender_multiplier)#defender gains points (double points if player was defending)
		...
		if "vamp" in card[4]:#Attacker has vamp status
			...
			card[0] += hp_regen
			if hp_regen > 0:
				self.battle_log.append("{} regained {} HP from vamp".format(card[5],hp_regen))#Dont show this message if no healing is done
