...
class rewards(commands.Cog):
	...
	async def change_image(self,ctx,ID:int,Image = 0):
		...
		if Image == 0:
			...
			confirm_embed = discord.Embed(
				...
				description = "<@{}> are you sure you want to change your card with ID: {} Into the below image? \nThis action can not be reversed without another Image change pass.\nClick on the green check mark to confirm.\n\nNote: If you trade this card away or sort your inventory before clicking the confirm button the image transfer may fail. Failure of transfer will result in the return of your image pass, however unforseen side effects may occur. Please message @Narwaffles#0927 if you encounter a seroius issue".format(ctx.author.id, ID+1)
				)
	...
	async def daily(self,ctx):
		...
		embed.add_field(name = "You have claimed your daily reward voucher.\nYou can claim your next one in 22 hours",value = "*"+random.choice(self.quotes)+"*")
	...
	async def daily_error(self,ctx,error):
		embed = discord.Embed(
			...
			colour = discord.Color.from_rgb(255,0,0),
			description = "You can only run this command once every 22 hours. Please try again in {}s".format(int(error.retry_after))
		)
	...
	async def buy_select(self,ctx,*,name):
		try:
			...
			embed = discord.Embed(
				...
				color = 0xFF0000,
				description = "Failed to locate a wallet beloning to {}. Please start by using the +guess command".format(ctx.author.name)
			)
			await ctx.send(embed=embed)
			return
