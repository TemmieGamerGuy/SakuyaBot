...
from bot import get_charinfo, put_charinfo, get_playercoins, add_playercoins, add_playervouchers, add_playerpasses, \
    get_playervouchers, get_playerpasses, info_internal, create_card, c, create_button, create_actionrow, ButtonStyle, \
    ComponentContext
...
class rewards(commands.Cog):
    ...
    @commands.command(aliases=["changeimage", "imagechange"])
    ...
    async def change_image(self, ctx, ID: int):
        ...
        embed.add_field(name=fullcharacters[char_name][0] + " - " + char_name, value='1/' + str(len(lines)))
        embed.set_image(url=image)
        await ctx.send(embed=embed, components=[create_actionrow(
            create_button(style=ButtonStyle.grey, emoji="⏪", custom_id="doubleleftimageselect"),
            create_button(style=ButtonStyle.grey, emoji="⬅️", custom_id="leftimageselect"),
            create_button(style=ButtonStyle.grey, emoji="✔️", custom_id="checkimageselect"),
            create_button(style=ButtonStyle.grey, emoji="➡️", custom_id="rightimageselect"),
            create_button(style=ButtonStyle.grey, emoji="⏩", custom_id="doublerightimageselect"))])
    ...
    async def shop(self, ctx):
        ...
        with open("shop.png", "rb") as imagefile:
            ...
            msg = await ctx.send(file=image_file, embed=shop_embed, components=[create_actionrow(
                create_button(style=ButtonStyle.blue, emoji="1️⃣", custom_id="1shop"),
                create_button(style=ButtonStyle.blue, emoji="2️⃣", custom_id="2shop"),
				create_button(style=ButtonStyle.blue, emoji="3️⃣", custom_id="3shop"),
				create_button(style=ButtonStyle.blue, emoji="4️⃣", custom_id="4shop"))])
    ...
    async def buy_select(self, ctx, *, name):
        ...
        if vouchers >= 2:
            ...
            if row is not None:
                ...
                chardict = characters
            for key in chardict:
                if name_lower in [x.lower() for x in chardict[key]]:
                    char_id = key
                    break
            else:
            ...
            msg = await ctx.send(embed=embed, file=img, components=[create_actionrow(create_button(style=ButtonStyle.blue, emoji="✅", custom_id="checklvl1image"))])
        else:
    ...
    async def on_raw_reaction_add(self, payload):
        if reaction.message.author.id == BOT_ID and user.id != BOT_ID:  # User reacted to a bot message and the user is not the bot
            ...
            if color == discord.Colour(8388736) and str(reaction) == "1\u20E3":
                if vouchers >= 1:
                    row = c.execute("SELECT * FROM guild_settings WHERE guildID=?",
                                    (reaction.message.guild.id,)).fetchone()
                    if row is not None:
                        if row[2] == 1:
                            chardict = fullcharacters
                        else:
                        ...
                        chardict = characters
                    char = random.choice(list(chardict))
    ...
    async def on_component(self, interaction: ComponentContext):
        ...
        if embed.title == "Select Image":
            ...
            if user.id != userid:
                return
            if interaction.component_id == 'leftimageselect':
                ...
                embed.set_field_at(0, name=embed.fields[0].name, value=str(imgnum) + '/' + str(len(lines)))
                embed.set_image(url=image)
            elif interaction.component_id == 'doubleleftimageselect':
                await interaction.edit_origin()
                char_name = embed.fields[0].name.split(" - ")[1]
                directory = char_dir + "//" + char_name
                with open(directory + '//imagelist.txt', "r") as f:
                ...
                embed.set_field_at(0, name=embed.fields[0].name, value=str(imgnum) + '/' + str(len(lines)))
                embed.set_image(url=image)
            elif interaction.component_id == 'doublerightimageselect':
                await interaction.edit_origin()
                char_name = embed.fields[0].name.split(" - ")[1]
                directory = char_dir + "//" + char_name
                with open(directory + '//imagelist.txt', "r") as f:
                ...
                embed.set_field_at(0, name=embed.fields[0].name, value=str(imgnum) + '/' + str(len(lines)))
                embed.set_image(url=image)
            elif interaction.component_id == 'checkimageselect':
                if get_playerpasses(userid) >= 1:
                    ...
                    await interaction.send(content="You do not have an image change pass to do that", hidden=True)
                    return
            await message.edit(embed=embed)
        vouchers = get_playervouchers(user.id)
        color = embed.color
        compid = interaction.component_id
        if compid == "1shop":
            if vouchers >= 1:
                try:
                    row = c.execute("SELECT * FROM guild_settings WHERE guildID=?", (message.guild.id,)).fetchone()
                    if row is not None:
                        if row[2] == 1:
                            chardict = fullcharacters
                        else:
                ...
                await interaction.send(embed=responce)
                await info_internal(message.channel, user.id, index)
            else:
                await self.not_enough(message.channel)
        elif compid == "2shop":
            if vouchers >= 1:
                responce = discord.Embed(
                    title="Select desired character",
            ...
            if vouchers >= 5:
                ...
                await self.not_enough(message.channel)
        elif compid == "checklvl1image":  # check mark for select level 1 image
            target_user, char = embed.footer.text.split(":")
            if user.id == int(target_user):
                if vouchers >= 2:
                    add_playervouchers(user.id, -2)
                    ...
                    await interaction.send("Not enough vouchers")

        elif compid == 'checkimagechange':  # check mark for image change
            target = embed.description.split(" ")[0]
            target = target.replace("<@", "").replace(">", "")
            target = int(target)
            if target == user.id:  # person reacted is the expected user
        ...
        elif compid == "ximagechange":  # x mark for image change
            target = embed.description.split(" ")[0]
            target = target.replace("<@", "").replace(">", "")
            target = int(target)
            if target == user.id:  # person reacted is the expected user
