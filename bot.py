...
async def help(ctx, *,command=None):
	thumb = ["https://i.imgur.com/javeshe.gif","https://i.imgur.com/BiaSHWW.jpg","https://i.imgur.com/7nCvHVq.jpg","https://i.imgur.com/eMmTjJy.jpg","https://i.imgur.com/gY9g699.jpg","https://i.imgur.com/fitJnaz.png"]
	...
	if command == None:
		...
		embed.add_field(name="Help Categories",value="• Touhou Cards\n• Trading\n• PvP\n• Combat\n• Rewards\n• Misc\n• New")
		embed.set_footer(text="[*] symbol means everything including spaces are included in that parameter. [= x] means that parameter defaults to that value and does not need an input from the user")
...
async def invite(ctx):
	await ctx.send("https://discord.com/api/oauth2/authorize?client_id=864237884473999382&permissions=117824&scope=bot%20applications.commands")
...
async def feed_stat(ctx,char,ID,amount,feed_type):
	...
	char_embed = discord.Embed(
		...
		description = "{} {} has been consumed".format(amount,["HP Pill(s) <:hp_pill:804830735982002176>","Attack Pill(s) <:attack_pill:804830653862117456>","Speed Pill(s) <:speed_pill:804830692126097429>"][feed_type])
		)
	
	char_embed.add_field(name = "Level:		",value = str(char[3]),inline = False)
	char_embed.add_field(name = "HP:		",value = str(char[5])+"/{}		(Current Stat/ Current Cap)".format(stat_cap[0]),inline = True)
	char_embed.add_field(name = "ATK:		",value = str(char[6])+"/{}		(Current Stat/ Current Cap)".format(stat_cap[1]),inline = True)
	char_embed.add_field(name = "SPD:		",value = str(char[7])+"/{}		(Current Stat/ Current Cap)".format(stat_cap[2]),inline = True)
...
async def feed_coin(ctx,char,ID,amount):
	...
	if coins_owned < amount:
		...
		await ctx.send("You do not have a wallet or any characters. Please get a character using the +guess command first before you use this command")
		return None
	...
	xp_bar = "		["+("█"*bar)+("-"*(10-bar))+"] "+str(current_xp)+"/"+str(xp_toLvl)+" (+{})".format(amount)
...
async def sort(ctx,option,reverse=True):
	...
	global char_info
	try:
		char_info[ctx.message.author.id].sort(key=lambda x:x[["character","image","name", "level", "rarity","hp", "attack", "speed","favourite"].index(option.lower().strip())], reverse=reverse)

		await ctx.send("Sort complete")
	except Exception as e:
		await ctx.send("Please follow the command with one of the following option to sort by: 'character', 'image', 'name', 'level', 'rarity', 'HP', 'Attack', 'Speed', favourite") 


@sort.error
async def sort_error(ctx, error):
	await ctx.send("Please follow the command with one of the following option to sort by: 'character', 'image', 'name', 'level', 'rarity', 'HP', 'Attack', 'Speed', favourite. You may also type (false) after the sort option to change it into ascending order. (Defaults to descending order)")
...
async def filter(ctx,*,options):
		...
		if greater >= 0:
			...
			if type(verify) == int:
				if verify == 0:#character name
					...
					if op[1] in ["true","t","y","yes"]: value = True
					else: value = False
	...
	if num_cards > 200:
		await filter_raise_error(ctx,options,"Exceeded the maximum of 200 search results. Please use more constricting querries and consider using the +sort command for more general searching")
		return
	elif num_cards <= 0:
	...
	while len(filtered_cards) > 0:
		...
		inv_embed.set_footer(text="Displaying page "+str(page+1)+" out of "+str(math.ceil(num_cards/CARDS_PER_PAGE))+" page(s)")
...
def gen_filtered_inv(card_list,amount):
	...
	for i in range(amount):
		try:
			char = card_list[i]
			line = ":star: "*char[8] + str(char[11]+1) + "	| " + str(char[2]) + "	| Lv" + str(char[3]) + "	| " + str(char[5]) + "	| " + str(char[6]) + "	| " + str(char[7]) + "	| " + ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][char[4]-1] +"\n"
			cards+=line
		except Exception as e:
			break
...
async def rename(ctx,id,*,name):
	...
	try:
		...
		await ctx.send("You do not have a character with the same ID\nYou must use this command as follows +rename <id (number)> <name (you can have spaces)> without the brackets")

def generate_char(character, image_name,identifier,level=0,weights = [70,20,7,2,1]):#Character refers to character code and image_name is the file name (make sure to strip the extension)
	rarities = [1,2,3,4,5]
...
async def create(ctx,user,folder,rarity,lvl,hp,atk,spd,*,img):
	...
	if ctx.message.author.id == OWNER_ID:
		...
		for char in char_info[int(user)]:
			if char[10] > max:
				max = char[10]
		char_info[int(user)].append([folder,img,characters[folder][0],int(lvl),int(rarity),int(hp),int(atk),int(spd),False,0,max+1])
...
def create_card(user,character,image,lvl,weights = [70,20,7,2,1]):#Non command version of create (kinda?). Used for rewards from shop
	global char_info
	max = 0
	...
	for trades in trade_inst:
		for i in range(2):
			if trades[i][0] == sender:
				await ctx.send("You are already in a trade. Please finish your current trade session before starting a new one")
				return False
			elif trades[i][0] == target:
				await ctx.send("The person you are trying to trade with is currently trading with someone. Please wait until they finish before starting a new trade")
				return False
	return True
...
async def test_img(ctx,id1,id2):
	global char_info
	im = generate_trade_img(char_info[ctx.message.author.id][int(id1)-1],char_info[ctx.message.author.id][int(id2)-1])
...
async def trade(ctx,user:discord.Member):
	...
	trade_embed = discord.Embed(
		...
		description = "<@{}>, <@{}> has requested to trade with you. React with \u2705 to start trading.".format(user.id,sender.id)
	)
...
async def offer(ctx, id=0):
	...
	#checks
	if trade_info == []:
		await ctx.send("You are not currently in a trade. Use +trade user to start a trade with someone before using this command")
		return
	...
	if trade_info[0][1][1] == None: line1 = "Nothing"
	else: line1 = "Name: "+str(trade_info[0][1][1][2]) + "	| Lv: " + str(trade_info[0][1][1][3]) + "	| HP: " + str(trade_info[0][1][1][5]) + "	| ATK: " + str(trade_info[0][1][1][6]) + "	| SPD: " + str(trade_info[0][1][1][7]) + "	| Rarity: " + ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][trade_info[0][1][1][4]-1]
	
	if trade_info[1][1][1] == None: line2 = "Nothing"
	else: line2 = "Name: "+str(trade_info[1][1][1][2]) + "	| Lv: " + str(trade_info[1][1][1][3]) + "	| HP: " + str(trade_info[1][1][1][5]) + "	| ATK: " + str(trade_info[1][1][1][6]) + "	| SPD: " + str(trade_info[1][1][1][7]) + "	| Rarity: " + ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][trade_info[1][1][1][4]-1]
	...
	try:
		...
		if (color == discord.Colour(65534)) and owner == str(user):#inventory scrolling
			...
			new_embed = discord.Embed(
				...
				description = "Points: {} <:point:795490918563840011>\nVouchers: {} :tickets:".format(player_coins[user.id][0],player_coins[user.id][1])
				)	
			...
			if cards == "":
				await reaction.message.channel.send("You do not have any characters on that page")
				return None

			new_embed.add_field(name = "	ID	|		Nickname		|	Level	|	HP	|	ATK	|	SPD	|	Rarity	|",value=cards)
			new_embed.set_footer(text="Displaying page "+str(current_page+1)+" out of "+str(math.ceil(len(user_chars)/12))+" page(s)")
			...
			if target == str(user.id):#person who reacted is the person being requested
				...
				if await check_trading(reaction.message.channel,int(sender),int(target)):
					new_embed = discord.Embed(
						title = "Trade Window",
						colour = discord.Color.from_rgb(253,255,0),
						description = "Type in +offer {ID} to put a card up for trade. Press \u2705 when you have confirmed that you are ok with the contents being traded. If there is a change in the offer, you must reconfirm the offer. Press :x: to cancel the trade. Note that you must either complete the trade or cancel it to start a new trade. Trade sessions do not expire"
					)
					new_embed.add_field(name = str(client.get_user(int(sender))), value="Nothing",inline = False)#god this is so stupid. I get the user, convert it to an id and get the user info back using this command. Im too lazy to do this properly but this is just getting worse and worse
					new_embed.add_field(name = "--------------------------------------------------------------", value="--------------------------------------------------------------",inline = False)
					...
					msg = await reaction.message.channel.send(file=img, embed=new_embed)
				
					trade_inst.append([[int(sender),[None,None],False],[int(target),[None,None],False],msg])#starts trade session
			...
			if trade_info != [] and str(reaction) == u'\u2705':#check mark (confirm trade)
				if trade_info[i][1][1] == None:
					await reaction.message.channel.send("You must send a card before you can confirm the trade")
					return
				trade_info[i][2] = True
				new_embed = embed
				...
				if trade_info[0][2] and trade_info[1][2]:
					fail1 = False
					fail2 = False
					new_embed.color = discord.Color.from_rgb(0,255,0)#I swear this is going to cause problems in the future but meh. let my future self suffer for this autism
					...
					if not fail1:
						...
						new_embed.description = "Failed to find card belonging to {}. Trade has been canceled".format(str(client.get_user(int(trade_info[0][0]))))
					...
					if fail2:#give back card
						...
						new_embed.title = "Trade Canceled"
						new_embed.description = "Failed to find card belonging to {}. Trade has been canceled".format(str(client.get_user(int(trade_info[1][0]))))
						new_embed.color = discord.Color.from_rgb(255,0,0)
				...
				if reaction.message.id == trade_info[2].id:#compares message ids
					trade_inst.remove(trade_info)#deletes trade instance
...
def gen_inv(user_chars,page):
	...
	for i in range(12):
		try:
			...
			line = ":star: "*char[8] +str(i + (page*12) + 1) + "	| " + str(char[2]) + "	| Lv" + str(char[3]) + "	| " + str(char[5]) + "	| " + str(char[6]) + "	| " + str(char[7]) + "	| " + ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][char[4]-1] +"\n"
			cards+=line
		except Exception as e:
			break
...
async def server(ctx):
	...
	await ctx.author.send("https://discord.gg/CMsz9qA")
...
async def inventory(ctx, page=1):
	...
	user_chars = char_info.get(ctx.message.author.id)
	if user_chars == None:
		await ctx.send("Sorry {} , you do not have any character cards yet. Try getting one by using the +guess command".format(ctx.message.author))
		return None
		
	inv_embed = discord.Embed(
		title = "Your Touhou Cards:",
		colour = discord.Color.from_rgb(0,255,254),
		description = "Points: {} <:point:795490918563840011>\nVouchers: {} :tickets:".format(player_coins[ctx.author.id][0],player_coins[ctx.author.id][1])
	)
...
async def sell(ctx, target, *, stars=0):
	...
	if target == "bulk":#Bulk selling
		...
		if stars <= 0 or stars >= 7:
			await ctx.send("Please input a valid rarity. You must use the command as +sell bulk {rarity} when bulk selling")
			return None
		try:
		...
		suc_embed = discord.Embed(
				...
				colour = discord.Color.from_rgb(0,255,255),
				description = "You have earned {} <:point:795490918563840011> from selling ".format(total_value) + str(len(sell_chars)) + " characters."
			)
		...
		if char[8]:
			await ctx.send("You cannot sell characters you have favorited. Please unfavorite by typing +favorite {ID} and try selling the character again")
			return None
		else:
...
async def sell_error(ctx, error):
	await ctx.send("Please input a valid rarity or ID. You must use the command as +sell bulk {rarity} when bulk selling")

async def info_internal(channel,owner,id):
	global char_info
	
	rare_img = ["https://i.imgur.com/i9MbAc4.png","https://i.imgur.com/VCGR1iu.png","https://i.imgur.com/fD1E57X.png","https://i.imgur.com/hohZnFi.png","https://i.imgur.com/Sx3vB8n.png","https://i.imgur.com/4Xe5Jg3.gif"]
	...
	bar = int(progress//10)
	xp_bar = " ["+("█"*bar)+("-"*(10-bar))+"] "+str(current_xp)+"/"+str(xp_toLvl)+" ({:.1f}%)".format(progress)
	char_embed.add_field(name = "Rarity:	",value = ["★☆☆☆☆","★★☆☆☆","★★★☆☆","★★★★☆","★★★★★","★★★★★★"][user_chars[id][4]-1],inline = False)
...
async def info(ctx, id=1):
	...
	user_chars = char_info.get(ctx.message.author.id)
	if user_chars == None:
		await ctx.send("Sorry {}, you do have any character cards yet. Try getting one by using the +guess command".format(ctx.message.author))
		return None
	...
	try:
		...
		card = ["Generic Spell Card","single_target_dmg",100,True,10,"A spell given to characters without a spell card. Deals damage to a single target",0.4]
	
	try:
		Power = "\nPower: {}".format(int(float(card[6])*100))
	except:
		Power = ""
	char_embed.add_field(name = "<:card:797943200907001867> - {} - {}<:power:796499186106236960>".format(card[0],card[2]), value = "Priority: {}".format(card[4]) + Power + "\nDesc: {}".format(card[5]),inline = False)
...
async def guess(ctx):
	...
	if (ctx.channel.id in guess_inst):#checks if channel is in list of active guess instances
		await ctx.send("Guess instance already active, please wait until previous instance ends until starting a new one.")
		return None
	else:
	...
	try:
		...
		#print(e)
		await ctx.send(content = "Uh oh! Looks like you're all out of time. Better luck next time\nThe Characters name is "+sol[0])
		score = [score[0],score[1]+1]
		
	#await ctx.send("This character has a " + str(int(score[0]/score[1] * 100)) +"% guess rate")
	
	char_save[target] = score
	while ctx.channel.id in guess_inst: guess_inst.remove(ctx.channel.id)#Remove all instances of channel id in guess_inst for use next time command is called
